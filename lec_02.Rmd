...
- [Functions](https://warpwire.duke.edu/w/yRcFAA/)
...
- Integers must be followed by an L, using `:` is an exception.
    ```{r results='hold'}
    x <- 1:100
    y <- as.double(1:100)
    c(typeof(x), typeof(y))
...
Previously, we looked at R's coercion hierarchy when creating atomic vectors:

.center[
`logical` < `integer` < `double` < `character`, 
]

where logical is considered the "simplest".
...
- Coercion is possible with the `as.*()` family of functions.
...
- Note that current implementations of R use 32-bit integers for 
  integer vectors, so the range of representable integers is restricted to 
  about $\pm 2 \times 10^9$; doubles can hold much larger integers exactly.
...
- Matrices, arrays, factors, and date-times are built on top of atomic
  vectors by adding attributes. We'll get more into this when we cover
...
```{r}
list("A", c(TRUE, FALSE), (1:4) / 2, function(z) z^2)
```
...
```{r}
str(list("A", c(TRUE, FALSE), (1:4) / 2, function(z) z^2))
```

Using `View()` in your console is another good option.

---

## Creation, coercion, and testing
...
John McKinley Chambers is the creator of the S programming language, and core 
member of the R programming language project.

---

## References

1. Grolemund, G., & Wickham, H. (2021). R for Data Science. https://r4ds.had.co.nz/
