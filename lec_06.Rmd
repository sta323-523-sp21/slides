...
3. Choose a subset of five variables and 10 rows from `wake` and save it 
...
## Useful `summarise()` helper functions
...
Suppose we want to break the Wake county parcel sizes into three categories:
small, medium, large. 
...
# New and advanced functionality

---

## `across()` examples

Apply a function, or set of functions, to columns.
...
flights %>% 
  ...
  group_by(day) %>% 
  summarise(across(c(dep_delay, arr_delay), ~median(., na.rm = TRUE)))
```
]
...
flights %>% 
  summarise(across(where(is.double), min_max))
```
]
...
Prior versions of dplyr allowed you to apply a function to multiple columns in 
a different way: using functions with `_if`, `_at`, and `_all()` suffixes. 
These functions are now superseded.

---

## More `select()` - matching
...
See `?select` for information on other helpers: 
